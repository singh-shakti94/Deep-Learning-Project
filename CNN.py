...
train_data = np.array(data[["band_1", "is_iceberg"]])

# converting labels to one-hot vector
b = np.zeros((1604, 2))
b[np.arange(1604), list(train_data[:, 1])] = 1
train_data[:, 1] = list(b)
...
# Network Parameters
num_input = 5625  # flattened band_1 data input (image shape: 75*75)
...
class CNN:
    ...
    def train(self, train_data, train_targets, batch_size=1, epochs=1):
        sess = self.session
        batches_x = np.array_split(train_data, int(len(train_data) / batch_size), axis=0)
        batches_y = np.array_split(train_targets, int(len(train_data) / batch_size), axis=0)
...
c = CNN()
c.train(train_data[:, 0], train_data[:, 1], batch_size=10, epochs=2)